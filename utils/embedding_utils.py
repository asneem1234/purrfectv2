"""
Embedding utilities wrapper module.
This module redirects imports to either the full or lite implementation
...
logger = logging.getLogger("embedding_utils_wrapper")

# Try to import from the full implementation first
try:
    # Check if we have sentence_transformers available
    st_spec = importlib.util.find_spec('sentence_transformers')
    qdrant_spec = importlib.util.find_spec('qdrant_client')
    
    if st_spec and qdrant_spec:
        # If full dependencies are available, use the full implementation
        logger.info("Using full embedding utilities implementation")
        # These would be imported from the full module
        pass
    else:
        raise ImportError("Missing required dependencies")
        
except (ImportError, ModuleNotFoundError):
    # If dependencies are not available, use the lite implementation
    logger.info("Using lite embedding utilities implementation")
    # Using simplified mock implementations
    pass
